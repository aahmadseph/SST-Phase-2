...
import com.sephora.services.inventory.mapper.InventoryShipNodeMapper;
...
import com.sephora.services.inventory.repository.cosmos.InventoryCosmosShipNodeRepository;
...
@Service
public class InventoryShipNodeService {
    ...
    private InventoryCosmosShipNodeRepository shipNodeRepository;

    @Autowired
    private InventoryShipNodeMapper shipNodeMapper;

    @Autowired
    private InventoryShipNodeService self;
     ...
     public void updateShipNodesStatus(List<String> shipNodeKeys, ShipNodeStatusEnum status) {
        ...
        if (CollectionUtils.isEmpty(foundShipNodes)) {
            logger.error("Unable to find shipNodes={}", shipNodeKeys);
            ...
            logger.warn("Unable to find shipNodes={}", notFoundShipNodes);
        }
        ...
        logger.debug("ShipNodes={} status updated to '{}'",
    ...
    @Cacheable(cacheNames = SHIP_NODE_CACHE_NAME, key = "'id:' + #shipNodeKey", unless = "#result == null", cacheManager = "cache2kCacheManager")
    public Optional<ShipNode> getShipNodeByKey(String shipNodeKey) {
        try {
            ...
            logger.error ("Error during retrieve shipNode by shipNodeKey {}", shipNodeKey, exception);
        }
        return Optional.empty();
    }

    @Cacheable(cacheNames = GET_SHIP_NODE_BY_ENTERPRISE_CODE, key = "#enterpriseCode", unless = "#result == null or #result.size()==0", cacheManager = "cache2kCacheManager")
    ...
    @Caching(evict = {
            @CacheEvict(cacheNames = SHIP_NODE_CACHE_NAME, key = "'id:' + #shipNode.id", cacheManager = "cache2kCacheManager"),
            @CacheEvict(cacheNames = GET_SHIP_NODE_BY_ENTERPRISE_CODE, key = "#shipNode.enterpriseCode", cacheManager = "cache2kCacheManager")
    })
    public void cacheEvict(ShipNode shipNode) {
        logger.debug("Evict all caches for shipNode {}", shipNode);
    }
