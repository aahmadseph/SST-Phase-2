...
describe('bvToken', () => {
    ...
    beforeEach(() => {
        bvToken = require('services/api/sdn/sdnToken').default;
    ...
    describe('getSdnToken', () => {
        ...
        it('should return token from storage if present', () => {
            ...
            bvToken.getSdnToken('BV_AUTH_TOKEN').then(data => {
                expect(data).toEqual('stored_token');
            });
            expect(getItemStub).toHaveBeenCalledTimes(1);
            expect(getItemStub).toHaveBeenCalledWith(LOCAL_STORAGE.BV_AUTH_TOKEN);
        });
        ...
        it('should call remote and put token into storage', () => {
            const fakePromise = {
                then: resolve => {
                    ...
                    expect(setItemStub).toHaveBeenCalledWith(LOCAL_STORAGE.BV_AUTH_TOKEN, responseStub.sdnAccessToken, responseStub.expiresIn * 1000);
                }
            };

            getItemStub.and.returnValue(null);
            getSdnAuthTokenStub.and.returnValue(fakePromise);
            bvToken.getSdnToken('BV_AUTH_TOKEN');
            expect(getSdnAuthTokenStub).toHaveBeenCalledTimes(1);
            expect(getSdnAuthTokenStub).toHaveBeenCalledWith({ clientName: getSdnAuthToken.SDN_AUTH_CLIENT_NAME_BV });
        });
    });
    ...
    describe('dropSdnToken', () => {
        it('should remove item from the local storage', () => {
            ...
            bvToken.dropSdnToken('BV_AUTH_TOKEN');
            expect(removeItemStub).toHaveBeenCalledWith(LOCAL_STORAGE.BV_AUTH_TOKEN);
        });
    });

    describe('callWithSdnToken', () => {
        let deepExtend;
        ...
        beforeEach(() => {
            ...
            dropSdnTokenStab = spyOn(bvToken, 'dropSdnToken');
            getSdnTokenStub = spyOn(bvToken, 'getSdnToken');
        });

        it('should callFunc once on 200 and dropBVToken', () => {
            ...
            bvToken.withSdnToken(aStub)(...args);
